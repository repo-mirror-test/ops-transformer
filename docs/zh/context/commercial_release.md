...
```
\$\{ops\_project\}表示待修改项目（如ops-math、ops-nn、ops-transformer等）。
...
2. **安装自定义算子包。**
    ...
    自定义算子包安装路径为`${ASCEND_HOME_PATH}/vendors`，\$\{ASCEND\_HOME\_PATH\}已通过环境变量配置，表示CANN toolkit包安装路径，一般为\$\{install\_path\}/cann。注意自定义算子包不支持卸载。
