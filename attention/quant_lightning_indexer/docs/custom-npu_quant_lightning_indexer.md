...
`QuantLightningIndexer`在[LightningIndexer](./custom-npu_lightning_indexer.md)的基础上支持了[Per-Token-Head量化](../../../docs/models/deepseek-v3.2-exp/deepseek_v3.2_exp_inference_guide.md)输入。对于某个 token 对应的 Index Query $Q_{index}^{INT8}\in\R^{g\times d}$ 及其反量化系数$Scale_{Q}\in\R^{g\times 1}$，给定上下文 Index Key $K_{index}^{INT8}\in\R^{S_{k}\times d}$ 及其反量化系数$Scale_{K}\in\R^{S_{k}\times 1}$, $W\in\R^{g\times 1}$，其中 $g$ 为 GQA 对应的 group size，$d$ 为每一个头的维度，$S_{k}$ 是上下文的长度，`QuantLightningIndexer`的具体计算公式如下：
$$
...
-   详见[test_npu_quant_lightning_indexer.py](../examples/test_npu_quant_lightning_indexer.py)